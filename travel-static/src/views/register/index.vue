  ...
  <div>
    <form model="registerForm" class="register">
      ...
      <div class="input-wrap">
        <input v-model="registerForm.email" name="email" v-validate="'required|email'" type="text" placeholder="您的邮箱地址" maxlength="28" />
      ...
      <div class="input-wrap" v-if="isSendCode">
        <input v-model="registerForm.password" name="password" v-validate="'required|alpha_num|min:6|max:32'" type="password" placeholder="您的密码" maxlength="32" @keyup.enter="register"/>
...
export default {
  ...
  computed: {
    encryptPassword: function () {
      return md5(this.registerForm.password);
    }
  },
  mounted() {
  },
  methods: {
    ...
    register() {
      ...
      this.$validator.validateAll().then((flag) => {
        ...
        this.registerForm.password = this.encryptPassword;
        request.post("/user/register", this.registerForm).then(e => {
          let _this = this;
          if (e.err) {
            Toast(e.err);
            _this.registerLock = false;
          } else {
            Toast("注册成功!");
            setTimeout(function () {
    ...
    sendValidateCode() {
      ...
      this.$validator.validate("email").then((flag) => {
        ...
        request.post("/user/sendCode", _this.registerForm).then(e => {
          if (e.err) {
            Toast(e.err);
            initSendCode.call(_this);
          } else {
